...
#define BH1750_SENSOR_ADDR CONFIG_BH1750_ADDR   /*!< slave address for BH1750 sensor 0x48*/
#define BH1750_CMD_START CONFIG_BH1750_OPMODE   /*!< Operation mode */
#define TMP117_SENSOR_ADDR 0x48   /*!< slave address for tmp117 0x48*/
...
static esp_err_t i2c_master_sensor_test(i2c_port_t i2c_num, uint8_t *data_h, uint8_t *data_l)
{
    ...
    i2c_cmd_handle_t cmd = i2c_cmd_link_create();
    i2c_master_start(cmd);
    i2c_master_write_byte(cmd, TMP117_SENSOR_ADDR << 1 | WRITE_BIT, ACK_CHECK_EN);
...
/*    i2c_master_stop(cmd);
    ...
    i2c_master_start(cmd);*/
    i2c_master_start(cmd);
    i2c_master_write_byte(cmd, TMP117_SENSOR_ADDR << 1 | READ_BIT, ACK_CHECK_EN);
...
*/
    ...
    while (1) {
        ...
        if (ret == ESP_ERR_TIMEOUT) {
            ...
            printf("*******************\n");
            printf("TASK[%d]  MASTER READ SENSOR( TMP117 )\n", task_idx);
            printf("*******************\n");
            printf("data_h: %02x\n", sensor_data_h);
            printf("data_l: %02x\n", sensor_data_l);
            printf("sensor val: %d [Lux]\n", (sensor_data_h << 8 | sensor_data_l) );
            printf("tmp_amb: %02f C\n", (sensor_data_h << 8 | sensor_data_l)*0.0078125);
        } else {
